...
<div class="wrap">
    ...
    <div class="vodlist_l box">
      <div class="top">
		<h3><a href="<?php echo ($root); ?>">首页</a></label> > <a href="<?php echo ($list_url); ?>"><?php echo ($list_name); ?></a></h3>
      </div>
      <?php $vod_list = ff_mysql_vod('cid:'.$list_id.';limit:8;page:true;order:vod_addtime desc,vod_id desc');$page = $vod_list[0]['page']; ?>
      <?php if(($vod_list["0"]["count"])  !=  "0"): ?><?php if(is_array($vod_list)): $i = 0; $__LIST__ = $vod_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ppvod): ++$i;$mod = ($i % 2 )?><ul><a href="<?php echo ($ppvod["vod_readurl"]); ?>"><img class="lazy" data-original="<?php echo ($ppvod["vod_picurl"]); ?>" src="" alt="点击播放《<?php echo ($ppvod["vod_name"]); ?>》" /></a>
        ...
        <p>地区∶<?php echo (($ppvod["vod_area"])?($ppvod["vod_area"]):'未录入'); ?> 更新时间∶<?php echo (date('Y-m-d',$ppvod["vod_addtime"])); ?></p>
    ...
    <div class="vodlist_r box">
        ...
        <ul class="list300"><?php echo getadsurl('list300');?></ul>
        <ul><?php $gold_desc = ff_mysql_vod('cid:'.$list_id.';limit:30;order:vod_gold desc,vod_hits desc'); ?><?php if(is_array($gold_desc)): $i = 0; $__LIST__ = $gold_desc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ppvod): ++$i;$mod = ($i % 2 )?><li><span><?php echo ($ppvod["vod_gold"]); ?></span><em><?php echo ($i); ?></em><a href="<?php echo ($ppvod["vod_readurl"]); ?>"><?php echo (msubstr($ppvod["vod_name"],0,12)); ?></a></li><?php endforeach; endif; else: echo "" ;endif; ?></ul>        
    </div>     
    <div class="blank"></div>
    <div class="ft">
    <div class="ft_l">
